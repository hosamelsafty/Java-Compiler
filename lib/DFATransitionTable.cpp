...
DFATransitionTable::DFATransitionTable() :
		m_d(std::make_unique<impl>()), startingState(NULL) {
...
State DFATransitionTable::nextState(const State &currentState,
		char input) const {
	/** Edited by OmarKassem. */
	// if the input is not exits for this state the operator[] will create a new element in the map
	// which is not required.
	// return m_d->table[currentState][input];
	// for the first map we are sure that the currentstate is already exits in the map
	// so there is no need to use find().
